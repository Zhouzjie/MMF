...
# 2022-09-04 22:57:22,144 Image to text: 73.4, 93.1, 96.6, 1.0, 2.3
# 2022-09-04 22:57:22,361 Text to image: 55.2, 80.7, 87.3, 1.0, 28.6
...
def validate(opt, val_loader, model):
    ...
    img_embs, cap_embs, img_sgrs, cap_sgrs, cap_lens = encode_data(
        model, val_loader, opt.log_step, logging.info)

    img_embs = numpy.array([img_embs[i] for i in range(0, len(img_embs), 5)])
    img_sgrs = numpy.array([img_sgrs[i] for i in range(0, len(img_sgrs), 5)])

    start = time.time()
    sims = shard_attn_scores(model, img_embs, cap_embs, img_sgrs, cap_sgrs, cap_lens, opt, shard_size=100)
    end = time.time()
